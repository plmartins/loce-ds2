 ...
 */
export function RailUser({ name, email, avatarUrl, loading, badge, children, className }: RailUserProps) {
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                ...
                >
                    <FaceAvatar seed={seed} glyph={name?.[0]} src={avatarUrl} loading={loading} title={name} size={28} />
                </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="right" align="end" sideOffset={10} className="w-64 p-1.5">
                <div className="flex items-center gap-3 rounded-lg bg-surface-1 p-2.5">
                    <FaceAvatar seed={seed} glyph={name?.[0]} src={avatarUrl} loading={loading} title={name} size={40} />
